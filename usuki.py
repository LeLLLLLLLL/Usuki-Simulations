...
psipm = np.zeros((islmax, rows, 2), dtype=np.complex128)
...
# Sort xm/imv descending
    ...
    # Copy matrices
        ...
        print('e,bmag,trans,nunits,error')
        print(en, bet, trans, float(nprop) - trans - ref)
    ...
    with open("waves.txt", "a") as wvs:
        for lplot in range(1, nsl + 2):  
            for m in range(rows):
                if psipm[lplot, m, 0] < 1e-10:
                    psipm[lplot, m, 0] = 1e-10
                if psipm[lplot, m, 1] < 1e-10:
                    psipm[lplot, m, 1] = 1e-10

                wvs.write(f"{psipm[lplot, m, 0]:.8e} {psipm[lplot, m, 1]:.8e}\n")
